...
def songs():
    songs = db.songs.find({})
    songs = json_util.dumps(list(songs))
...
def create_song():
    ...
    response = {"inserted id": {"$oid": str(result.inserted_id)}}
...
def update_song(id):
    ...
    update_result = db.songs.find_one({"id":id})
    response = json_util.dumps(update_result)
    return response, 201
